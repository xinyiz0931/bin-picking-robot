...
crop_mz = crop_roi(img_mz_path, cfg['drop']['margin'])
...
if res_mz is None: 
    ...
    if cls == 0:
        ...
        g_pull = action[0:3]
        g_hold = action[3:6]
        v_pull = [action[7], action[6]]
        p_fg_pull, g_r_pull = transform_image_to_robot(g_pull, point_array, cfg, hand="left", margin="drop",dualarm=True)
        p_fg_pull, g_r_hold = transform_image_to_robot(g_hold, point_array, cfg, hand="right", margin="drop", tilt=60, dualarm=True)
        ...
        # gen_motion_pickorsep(mf_path, g_r_pull, pulling=[*v_pull, v_len])
        #dual_arm = True
        # gen_success = gen_motion_pickorsep(mf_path, [rx,ry,rz,ra], dest="drop")
    success = True

end_t = timeit.default_timer()
# print("[*] Time: ", end_t -  end_t_capture)
main_proc_print(f"Success? => {success}")
