...
def draw_projection(root_dir, shape, euler_angle = [0, 0, 0]):
    main_proc_print("-----------------------------------------------")
    tc6d = TopoCoor6D()
    graph = []
    ...
    for i, j in zip(sorted_index, range(num_obj)):
        ...
        tc6d.draw_projection_node_2d(node, proj_obj_ea , ax2, alpha=1, color=node_cmap)
    ...
    for i, j in zip(sorted_index, range(num_obj)):
        ...
        node[:,1] = 0
        node_cmap = cmap(j)
        tc6d.draw_node(node, ax3d, alpha=0.3, color=node_cmap)
        # tc6d.draw_projection_node_new(node, proj_euler_angle , ax3d, alpha=0.4, color=node_cmap)
...
def main():
    ...
    # pick_idx = draw_projection(root_dir, shape)
    if pick_idx == -1:
