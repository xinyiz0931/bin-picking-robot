...
class PickAndPlaceActor(object):
    ...
    def get_place_seq(self, rpy, dest):
        if dest == "front":
            return [
                "0 0.80 LARM_XYZ_ABS %.3f %.3f 0.300 %.1f %.1f %.1f" % (*self.front_c,*rpy),
    ...
    def get_drop_seq(self, rpy):
        return [
            "0 1.00 LARM_XYZ_ABS %.3f %.3f 0.300 %.1f %.1f %.1f" % (*self.drop_bin,*rpy),
            "0 1.00 LARM_XYZ_ABS %.3f %.3f 0.250 %.1f %.1f %.1f" % (*self.drop_bin,*rpy),
            "0 0.50 LHAND_JNT_OPEN",
            self.initpose
        ]