...
class PickAndPlaceActor(object):
    def __init__(self, filepath=None, arm='R'):
        ...
        self.front_c = [0.430, 0.200]
        self.side_c = [0.000, 0.552]
        self.pick_bin = [0.430, -0.050]
        self.drop_bin = [0.430, 0.200]
        self.arm = arm
    ...
    def get_place_seq(self, rpy, dest):
        if dest == "front":
            return [
                "0 0.80 "+self.arm+"ARM_XYZ_ABS %.3f %.3f 0.300 %.1f %.1f %.1f" % (*self.front_c,*rpy),
                "0 0.50 "+self.arm+"ARM_XYZ_ABS %.3f %.3f 0.250 %.1f %.1f %.1f" % (*self.front_c,*rpy),
                "0 0.50 "+self.arm+"HAND_JNT_OPEN",
                self.initpose
            ]
        elif dest == "side":
            return [
                ...
                "0 1.50 "+self.arm+"ARM_XYZ_ABS %.3f %.3f 0.350 %.1f %.1f %.1f" % (*self.side_c,*rpy),
                "0 0.50 "+self.arm+"ARM_XYZ_ABS %.3f %.3f 0.250 %.1f %.1f %.1f" % (*self.side_c,*rpy),
                "0 0.50 "+self.arm+"HAND_JNT_OPEN",
                self.initpose
            ]
    ...
    def get_drop_seq(self, rpy):
        return [
            "0 1.00 "+self.arm+"ARM_XYZ_ABS %.3f %.3f 0.350 %.1f %.1f %.1f" % (*self.drop_bin,*rpy),
            "0 0.80 "+self.arm+"ARM_XYZ_ABS %.3f %.3f 0.300 %.1f %.1f %.1f" % (*self.drop_bin,*rpy),
            "0 0.50 "+self.arm+"HAND_JNT_OPEN",
            self.initpose
        ]